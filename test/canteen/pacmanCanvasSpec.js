// var rewire = require("rewire");

describe('#main', function(){
  // var PacMan = rewire('../../app/public/js/pacman.js');
  // var pacman;
  //
  // beforeEach(function() {
  //   // PacMan = app.__get__('PacMan');
  //   pacman = new PacMan(document.getElementById('pacman').getContext('2d'));
  // })

  it('pacman is a circle and yellow', function() {
    var context = this.test.context;
    // draw stuff
    context.beginPath();
    context.arc(30, 20, 10, 0, 2 * Math.PI, false);
    context.fillStyle = 'yellow';
    context.fill();
    context.closePath();


    // return an md5 hash of the instruction stack
    // var hash = pacman.context.hash();

    // compare hash
    assert.equal(context.hash(), '66a3b8e224c3a79b936d0dd0f694eb3a');

    // clear the stack
    // pacman.context.clear();
  });
})
